...
Verification evidence (2026-08-26): 187 tests passed in the Compose API container with PostgreSQL. Fake-platform contract tests prove both adapters build shell-free argv, parse strict JSON, preserve API errors verbatim as `PublishError` reasons, validate media/titles/credentials up front, and clean temporary artifacts. The scheduling coordinator accepts only APPROVED clips, moves them through the documented SCHEDULE/PUBLISH lifecycle, assigns deterministic rule-based slots (`DailyWindowRule`), and dispatches due publications onto independent per-platform queues (`publish.instagram_reels`, `publish.youtube_shorts`). Failures keep state and reason on the publication record and reschedule with capped exponential backoff inside a five-attempt budget; exhausted publications stay FAILED and refuse further retries. Publication records persist through migration `0007_publication_records`; migrations cycled cleanly with `alembic check` reporting no new operations. Ruff and mypy passed. Real network publishing remains strictly opt-in: adapters activate only when platform credentials are configured. Each phase gate above verified its components in isolation; end-to-end operability across those components is verified separately by the operational core below.

## Operational core — end-to-end durability

Status: verified

Depends on: Phases 0–6 verified.

Motivation: the per-phase gates verified each component in isolation, but a repository audit found that the components were not connected into a working pipeline — the API created database job rows that never reached Redis, the default Compose stack shared no media volume or credentials, and publishing had no dispatcher. This phase makes one authenticated upload or YouTube URL run durably from ingestion through transcription, selection, and 9:16 rendering.

Deliverables: deployable shared runtime (named `media-data`/`model-cache` volumes, `.env` propagation, `python-multipart`, `.[dev,transcription]` image); a transactional outbox (`outbox_events`, migration `0008_operational_core`) with a relay onto reliable Redis ready/processing lists (`BLMOVE`/`LREM` claims, receipts, acknowledgement); durable manual dispatch, `POST /jobs/{id}/retry`, duplicate-delivery suppression via row locks, and worker-start recovery of interrupted jobs; authenticated `POST /sources/upload` (bounded streaming, HTTP 413) and background `POST /sources/youtube` ingestion; automatic stage chaining (each successful stage dispatches its successor in the same transaction, gated on `auto_process`); and a public, non-gating `GET /system/transcription-readiness`.

Acceptance criteria: one upload reaches rendered reviewable clips with no manual queue writes; PostgreSQL is authoritative and Redis is at-least-once only; a duplicate delivery runs its handler exactly once; an interrupted job recovers on worker restart; tests never destroy the developer database or named volumes.

Verification gate: `scripts/verify.sh` — the full suite (including a real PostgreSQL/Redis/FFmpeg `upload → transcribe → select → render` integration test) plus Ruff, strict mypy, and an Alembic upgrade/check round-trip against a disposable database and reserved Redis logical database.

Verification evidence (2026-08-27): the full suite ran in the operational-core Docker image against a disposable `openclips_test_*` PostgreSQL database and reserved Redis logical database 15 — 246 passed, 1 skipped (the Compose-resolution test skips where the `docker` CLI is absent; it passes on the host), 1 documented pre-existing Starlette/httpx warning. The real-dependency `tests/integration/test_operational_pipeline.py` drove one authenticated upload through the outbox relay and worker to rendered reviewable clips: the source reached READY, a transcript persisted, and every clip reached READY_FOR_REVIEW at 1080×1920 with its rendered file on disk, using real PostgreSQL, Redis, and FFmpeg with only the transcription provider faked. `alembic upgrade head` then `alembic check` reported `No new upgrade operations detected`. Ruff and strict mypy passed (47 source files). See docs/VERIFICATION.md for the full record.
